...
class Graph:
  ...
  def explore(self):
    ...
    while current_room != 'treasure room':
        ...
        valid_choices = [item[0] for item in node.edges.keys()]
        print("\n You have accumulated {} cost".format(path_total))
        choice = input("\n Which room do you want to move to?\n")
        if choice not in valid_choices:
            print("\n Please select these letters: {}".format(valid_choices))
        else:
            for room in node.edges.keys():
                if room[0] == choice:
                    current_room = room
                    path_total += node.edges[room]
            print("\n ---You have chosen: {}---".format(current_room))
    print("\n ---- Made it to the treasure room with {} cost ----".format(path_total))
